...
[ipt2socks](https://github.com/zfl9/ipt2socks) 的重构增强版

## FakeDNS 增强

程序集成了深度优化的 **FakeDNS 模块**，专为透明代理环境打造。可作为通用的 **TProxy 前端**，为不支持 FakeDNS 的上游服务提供高性能映射支持。

## 功能特性

### 1. 确定性映射
不同于常规的“顺序递增”或“随机分配”策略，程序采用**确定性哈希算法**分配 IP。同一地址池内特定域名始终映射至唯一的 FakeIP。

### 2. 数据持久化
常规 FakeDNS 实现在重启后会丢失映射数据（某些支持持久化的实现在极端情况下也存在数据不一致的问题）。若客户端在服务重启后继续使用旧 FakeIP 发起连接，服务端会因映射数据缺失或不一致导致路由失败。本程序支持**数据持久化**：通过 `--fakedns-cache` 选项，程序在退出时将映射表保存至磁盘并在启动时加载。

### 3. 无感重启
得益于上述确定性映射与持久化机制，程序保证了客户端与服务端 DNS 数据的高度同步。服务重启后无需等待或刷新 DNS 缓存即可无缝接轨。

## 性能增强

在原版基础上，针对底层 IO 与内存管理进行了深度重构：
* **高效收发机制**：实现了多线程 UDP 和 `recvmmsg()` `sendmmsg()` 批量收发系统调用。
* **内存池管理**：使用定制化**内存池 (Memory Pool)** 取代原版的 `malloc` 动态内存分配，TCP 和 UDP 热路径均实现了零动态分配，有效减少内存碎片。

## 使用建议与场景说明

* **推荐场景**：使用 FakeDNS 解析特定域名列表（如 GFWlist），并通过 `iptables` 或 `nftables` 将 FakeIP 网段的流量导入 `ipt2socks`，正常流量完全不经过代理进程，实现精准分流。
* **不推荐场景**：将**全量** DNS 查询劫持到 FakeDNS 并将**所有流量**导入 `ipt2socks` 使用上游软件分流。由于哈希算法的可碰撞特性本方案不适用于处理全网海量域名。
...
类似 [redsocks](https://github.com/darkk/redsocks)、[redsocks2](https://github.com/semigodking/redsocks) 的实用工具，将 iptables/nftables TPROXY 传入的流量转为 socks5(tcp/udp) 流量，除此之外不提供任何不必要的功能。
