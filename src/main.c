...
static void print_command_help(void) {
    printf("usage: ipt2socks <options...>. the existing options are as follows:\n"
           ...
           " -c, --cache-size <size>            udp session cache maxsize, default: 256\n"
           " -o, --udp-timeout <sec>            udp session idle timeout, default: 60\n"
...
static void parse_command_args(int argc, char* argv[]) {
    opterr = 0;
    const char *optstr = ":s:p:a:k:b:B:l:S:c:o:j:J:n:TU46wWvVh";
...
static void* run_event_loop(void *arg) {
    ...
    size_t session_max_blocks = (size_t)udp_lrucache_get_fullcone_maxsize() +
                                (size_t)udp_lrucache_get_symmetric_maxsize() + 1;
    ...
    size_t fullcone_node_max_blocks = udp_lrucache_get_fullcone_maxsize() + 1;
    ...
    size_t symmetric_node_max_blocks = udp_lrucache_get_symmetric_maxsize() + 1;
    size_t symmetric_node_initial_blocks = MEMPOOL_INITIAL_SIZE;
    ...
    if (should_handle_udp) {
        /* 1. UDP Session Pool: serves udp_session_t only */
        ...
        /* 2. UDP TProxy Pool: serves udp_tproxy_entry_t only */
