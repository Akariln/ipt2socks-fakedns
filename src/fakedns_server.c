...
void fakedns_server_recv_cb(evloop_t *evloop, struct ev_watcher *watcher, int revents) {
    ...
    struct mmsghdr msgs[FAKEDNS_BATCH_SIZE];
    struct iovec iovecs[FAKEDNS_BATCH_SIZE];
    struct sockaddr_storage addrs[FAKEDNS_BATCH_SIZE];

    for (int i = 0; i < FAKEDNS_BATCH_SIZE; ++i) {
        iovecs[i].iov_base = g_fakedns_batch_packets[i].buffer;
        iovecs[i].iov_len = FAKEDNS_MAX_UDP_SIZE;
    ...
    for (int i = 0; i < nrecv; ++i) {
        ...
        if (nresp > 0) {
            send_msgs[send_count].msg_hdr.msg_name = msgs[i].msg_hdr.msg_name;
            send_msgs[send_count].msg_hdr.msg_namelen = msgs[i].msg_hdr.msg_namelen;
            send_msgs[send_count].msg_hdr.msg_iov = msgs[i].msg_hdr.msg_iov;
