...
/* handshake.resp[] must hold every SOCKS5 response received during handshake */
...
/* handshake.req[] must hold every SOCKS5 proxy request built during handshake */
...
static inline tcp_session_t* get_tcp_session_by_watcher(evio_t *watcher) {
...
static inline void tcp_session_release(evloop_t *evloop, tcp_session_t *session, bool is_tcp_reset) {
    ...
    evio_t *socks5_watcher = &session->socks5_watcher;
    ev_io_stop(evloop, client_watcher);
    ev_io_stop(evloop, socks5_watcher);
    ev_timer_stop(evloop, &session->handshake_timer);
    if (is_tcp_reset) {
    ...
    if (session->client_pipefd[0] != -1) {
        close(session->client_pipefd[0]);
    }
    if (session->client_pipefd[1] != -1) {
        close(session->client_pipefd[1]);
    }
    if (session->socks5_pipefd[0] != -1) {
        close(session->socks5_pipefd[0]);
    }
    if (session->socks5_pipefd[1] != -1) {
        close(session->socks5_pipefd[1]);
    }
...
void tcp_tproxy_accept_cb(evloop_t *evloop, struct ev_watcher *watcher, int revents __attribute__((unused))) {
    ...
    session->socks5_pipefd[0] = session->socks5_pipefd[1] = -1;
    session->prev = NULL;
    ...
    session->socks5_watcher.data = session;

    evio_t *io_watcher = &session->client_watcher;
    ev_io_init(io_watcher, tcp_stream_payload_forward_cb, client_sockfd, EV_READ);

    size_t actual_len = 0;
    socks5_proxy_request_make(session->handshake.req, &skaddr, fake_domain, &actual_len);
    session->handshake.req_len = (uint32_t)actual_len;

    io_watcher = &session->socks5_watcher;
    ...
    session->handshake.io_offset = (uint32_t)tfo_nsend;
    ev_io_start(evloop, io_watcher);

    session->handshake_timer.data = session;
    ev_timer_init(&session->handshake_timer, tcp_handshake_timeout_cb, TCP_HANDSHAKE_TIMEOUT_SEC, 0.);
    ev_timer_start(evloop, &session->handshake_timer);
}
...
static void tcp_socks5_connect_cb(evloop_t *evloop, struct ev_watcher *watcher, int revents __attribute__((unused))) {
    ...
    if (tcp_has_error(socks5_watcher->fd)) {
        ...
        tcp_session_release(evloop, get_tcp_session_by_watcher(socks5_watcher), true);
        return;
    }
...
static int tcp_socks5_send_request(const char *funcname, evloop_t *evloop, evio_t *socks5_watcher, const void *data, size_t datalen) {
    tcp_session_t *session = get_tcp_session_by_watcher(socks5_watcher);
    const uint8_t *pdata = (const uint8_t *)data;
    ssize_t nsend = send(socks5_watcher->fd, pdata + session->handshake.io_offset, datalen - session->handshake.io_offset, 0);
    if (nsend < 0) {
        if (errno != EAGAIN && errno != EWOULDBLOCK) {
    ...
    session->handshake.io_offset += (uint32_t)nsend;
    if (session->handshake.io_offset >= datalen) {
        session->handshake.io_offset = 0;
        return 1;
    }
    return 0;
}
...
static int tcp_socks5_recv_response(const char *funcname, evloop_t *evloop, evio_t *socks5_watcher, void *data, size_t datalen) {
    ...
    ssize_t nrecv = recv(socks5_watcher->fd, pdata + session->handshake.io_offset, datalen - session->handshake.io_offset, 0);
    if (nrecv < 0) {
        if (errno != EAGAIN && errno != EWOULDBLOCK) {
    ...
    session->handshake.io_offset += (uint32_t)nrecv;
    if (session->handshake.io_offset >= datalen) {
        session->handshake.io_offset = 0;
        return 1;
    }
...
static void tcp_socks5_recv_authresp_cb(evloop_t *evloop, struct ev_watcher *watcher, int revents __attribute__((unused))) {
    evio_t *socks5_watcher = (evio_t *)watcher;
    tcp_session_t *session = get_tcp_session_by_watcher(socks5_watcher);
    if (tcp_socks5_recv_response("tcp_socks5_recv_authresp_cb", evloop, socks5_watcher, session->handshake.resp, sizeof(socks5_authresp_t)) != 1) {
        return;
    }
    if (!socks5_auth_response_check("tcp_socks5_recv_authresp_cb", (const socks5_authresp_t *)session->handshake.resp)) {
        tcp_session_release(evloop, session, true);
        return;
    }
    const void *data = g_socks5_usrpwd_requestlen ? (const void *)&g_socks5_usrpwd_request : (const void *)session->handshake.req;
    size_t datalen = g_socks5_usrpwd_requestlen ? g_socks5_usrpwd_requestlen : session->handshake.req_len;
    ...
    if (ret == 1) {
        ...
        if (!g_socks5_usrpwd_requestlen) {
            session->handshake.resp_expect = SOCKS5_RESP_HEADER_PREFIX_LEN;
        }
    } else if (ret == 0) {
        ev_io_stop(evloop, socks5_watcher);
...
static void tcp_socks5_recv_usrpwdresp_cb(evloop_t *evloop, struct ev_watcher *watcher, int revents __attribute__((unused))) {
    evio_t *socks5_watcher = (evio_t *)watcher;
    tcp_session_t *session = get_tcp_session_by_watcher(socks5_watcher);
    if (tcp_socks5_recv_response("tcp_socks5_recv_usrpwdresp_cb", evloop, socks5_watcher, session->handshake.resp, sizeof(socks5_usrpwdresp_t)) != 1) {
        return;
    }
    if (!socks5_usrpwd_response_check("tcp_socks5_recv_usrpwdresp_cb", (const socks5_usrpwdresp_t *)session->handshake.resp)) {
        tcp_session_release(evloop, session, true);
        return;
    }
    int ret = tcp_socks5_send_request("tcp_socks5_recv_usrpwdresp_cb", evloop, socks5_watcher, session->handshake.req, session->handshake.req_len);
    if (ret == 1) {
...
static void tcp_socks5_send_proxyreq_cb(evloop_t *evloop, struct ev_watcher *watcher, int revents __attribute__((unused))) {
    evio_t *socks5_watcher = (evio_t *)watcher;
    tcp_session_t *session = get_tcp_session_by_watcher(socks5_watcher);
    if (tcp_socks5_send_request("tcp_socks5_send_proxyreq_cb", evloop, socks5_watcher, session->handshake.req, session->handshake.req_len) != 1) {
        return;
    }
    ev_io_stop(evloop, socks5_watcher);
...
static void tcp_socks5_recv_proxyresp_cb(evloop_t *evloop, struct ev_watcher *watcher, int revents __attribute__((unused))) {
    evio_t *socks5_watcher = (evio_t *)watcher;
    tcp_session_t *session = get_tcp_session_by_watcher(socks5_watcher);
    if (tcp_socks5_recv_response("tcp_socks5_recv_proxyresp_cb", evloop, socks5_watcher, session->handshake.resp, session->handshake.resp_expect) != 1) {
        return;
    }

    if (session->handshake.resp_expect == SOCKS5_RESP_HEADER_PREFIX_LEN) {
        uint8_t atype = ((socks5_resp_header_t *)session->handshake.resp)->addrtype;
        size_t total_len;
        ...
        if (total_len > sizeof(session->handshake.resp)) {
            LOGERR("[tcp_socks5_recv_proxyresp_cb] response too large: %zu", total_len);
            tcp_session_release(evloop, session, true);
            return;
        }

        session->handshake.resp_expect = (uint32_t)total_len;
        session->handshake.io_offset = SOCKS5_RESP_HEADER_PREFIX_LEN;
    ...
    if (!socks5_proxy_response_check("tcp_socks5_recv_proxyresp_cb", (const socks5_resp_header_t *)session->handshake.resp)) {
        tcp_session_release(evloop, session, true);
        return;
    }

    session->fwd.client_eof = false;
    session->fwd.socks5_eof = false;
    session->fwd.client_to_socks5_pending = 0;
    session->fwd.socks5_to_client_pending = 0;
    ...
    if (new_nonblock_pipefd(session->socks5_pipefd) < 0) {
...
static void tcp_stream_payload_forward_cb(evloop_t *evloop, struct ev_watcher *watcher, int revents) {
    ...
    tcp_session_t *session = get_tcp_session_by_watcher(self_watcher);
    bool self_is_client = (self_watcher == &session->client_watcher);
    evio_t *peer_watcher = self_is_client ? &session->socks5_watcher : &session->client_watcher;
    bool *self_eof = self_is_client ? &session->fwd.client_eof : &session->fwd.socks5_eof;
    bool *peer_eof = self_is_client ? &session->fwd.socks5_eof : &session->fwd.client_eof;
    uint32_t *self_pending = self_is_client ? &session->fwd.client_to_socks5_pending : &session->fwd.socks5_to_client_pending;
    uint32_t *peer_pending = self_is_client ? &session->fwd.socks5_to_client_pending : &session->fwd.client_to_socks5_pending;

    if (revents & EV_READ) {
        int *self_pipefd = self_is_client ? session->client_pipefd : session->socks5_pipefd;
...
DO_WRITE:
    if (revents & EV_WRITE) {
        int *peer_pipefd = self_is_client ? session->socks5_pipefd : session->client_pipefd;
    ...
    if (session->fwd.client_eof && session->fwd.socks5_eof &&
            session->fwd.client_to_socks5_pending == 0 && session->fwd.socks5_to_client_pending == 0) {
